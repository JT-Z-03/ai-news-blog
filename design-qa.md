...
- Copy and content: the reference title and description are unchanged, the body remains the authored DeepSeek analysis, and article metadata now renders as `2026 年 8 月 14 日 · 3 分钟 · 469 字 · ZestJT`. The breadcrumb is intentionally retained for orientation and was compacted so it does not displace the reference hierarchy. The source's illustrative 12% progress differs from the real top-of-page state, which correctly reports 0%.
