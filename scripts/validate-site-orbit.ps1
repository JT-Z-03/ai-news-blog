...
"@, $utf8WithoutBom)
    ...
    Require-Match $orbitCss '(?s)\.orbit-shell \.header-nav\s*\{[^}]*max-width:\s*min\(var\(--orbit-shell-width\),\s*calc\(100vw\s*-\s*32px\)\);' "The desktop header should use the same 1200px Orbit shell as the reference."
    Require-Match $orbitCss '(?s)\.orbit-reading-progress\s*\{[^}]*margin:\s*2\.6rem\s+auto\s+2\.5rem;' "Article progress should preserve the compact reference rhythm."
    Require-Match $orbitCss '(?s)\.orbit-article \.orbit-page__header \.breadcrumbs\s*\{[^}]*margin-bottom:\s*0\.75rem;' "Article breadcrumbs should retain semantics without displacing the reference hierarchy."
    Require-Match $orbitCss '(?s)\.orbit-article \.orbit-page__eyebrow\s*\{[^}]*margin-bottom:\s*1rem;' "The article category marker should keep the compact reference rhythm."
    Require-Match $orbitCss '(?s)\.orbit-article \.orbit-page__title\s*\{(?=[^}]*font-size:\s*clamp\(2\.4rem,\s*4\.7vw,\s*3\.5rem\))(?=[^}]*font-weight:\s*700)(?=[^}]*line-height:\s*1\.14)(?=[^}]*letter-spacing:\s*-0\.045em)[^}]*\}' "Article titles should match the reference scale, weight, width, and two-line rhythm."
    Require-Match $orbitCss '(?s)\.orbit-article__body\s*\{[^}]*font-size:\s*1\.1rem;' "Desktop article copy should match the reference reading density."
    Require-Match $orbitCss '(?s)\.orbit-list__row h2\s*\{[^}]*font-size:\s*clamp\(1\.5rem,\s*3vw,\s*2\.05rem\);' "Desktop list titles should avoid single-character orphan wraps."
    Require-Match $orbitCss '(?s)\.orbit-shell \.orbit-search__results a\.entry-link:focus-visible\s*\{(?=[^}]*outline:\s*3px\s+solid\s+var\(--orbit-accent\))(?=[^}]*outline-offset:\s*3px)[^}]*\}' "Keyboard-focused search results should retain the Orbit lime focus treatment."
    Require-Match $orbitCss '(?s)\.orbit-nav-deep-analysis \.menu a\[href\*="%E6%B7%B1%E5%BA%A6%E5%88%86%E6%9E%90/"\] span,[^{]*\{[^}]*box-shadow:\s*inset\s+0\s+-2px\s+var\(--orbit-accent\);' "Deep-analysis articles should retain the source navigation underline."
